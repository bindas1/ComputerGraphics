...
async function main() {
	...
	*/
	...
	const resources = {
		'shader_shadowmap_gen_vert': load_text('./src/shaders/shadowmap_gen.vert'),
		...
		'shader_ambient_vert':      load_text('./src/shaders/ambient_color.vert'),
		...
		'mesh_wheel': load_mesh_obj(regl, './meshes/shadow_scene__wheel.obj', {
	...
	*/
	...
	let is_paused = false;
	const toggle_pause = () => is_paused = !is_paused;
	document.getElementById('btn-pause').addEventListener('click', toggle_pause);
	register_keyboard_action('p', toggle_pause);

	let show_dist_map = true;
	const toggle_dist_map = () => show_dist_map = !show_dist_map;
	document.getElementById('btn-distance').addEventListener('click', toggle_dist_map);
	register_keyboard_action('g', toggle_dist_map);
...

	{
		...
		document.getElementById('btn-nocube').addEventListener('click', stop_vis_cube);
		register_keyboard_action('6', stop_vis_cube);
	}
